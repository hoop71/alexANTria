...
### ANT-* Layer Templates
Documentation templates for the 5-layer anthill structure.

- `ANT-SURFACE.md.template` → `<directory>/ANT-SURFACE.md` (individual service docs)
- `ANT-TUNNELS.md.template` → `ANT-TUNNELS.md` (architecture/service connections)
- `ANT-CHAMBERS.md.template` → `ANT-CHAMBERS.md` (cross-cutting patterns)
- `ANT-NEST.md.template` → `ANT-NEST.md` (product/business context)
- `ANT-QUEEN.md.template` → `ANT-QUEEN.md` (strategic alignment)

**Purpose:** Generated by `/ant-init` based on `starting_level` configuration. Worker ant auto-maintains files at or below `starting_level`.
...
### External Context Template
- `ANT-EXTERNAL.md.template` → `<directory>/ANT-EXTERNAL.md` (marks external context feeds)

**Purpose:** Marks directories containing externally-generated context (read-only).
